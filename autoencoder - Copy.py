...
class Autoencoder:
    """
    This class represents a deep convolutional autoencoder with mirrored encoder and decoder.
    """
    ...
    def compile(self, learning_rate=0.0001):
        ...
        self.model.compile(optimiser=optimiser, loss=mse_loss)
    ...
    def _add_dense_layer(self, decoder_input):
        num_neurons = np.prod(self._shape_before_flatten)
        dense_layer = Dense(num_neurons, name='dense_layer')(decoder_input)
        return dense_layer

    def _add_reshape_layer(self, dense_layer):
        return Reshape(self._shape_before_flatten)(dense_layer)
    ...
    def _add_conv_layer(self, layer_index, x):
        """
        add a convolutional bolck : conv 2d + ReLU + Batch Normalisation
        """
        ...
        x = conv_layer(x)
        x = ReLU(name=f'encoder_relu_{layer_index}')(x)
    ...
    def _add_bottleneck(self, x):
        ...
        """
        self._shape_before_flatten = K.int_shape(x)[1:]
        x = Flatten()(x)
